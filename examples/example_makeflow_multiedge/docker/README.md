# Dockerized Multi-Edge Makeflow Example

This directory contains the Docker configuration to run the [Multi-Edge Makeflow example](../README.md) in a fully containerized environment. This setup simulates a distributed workflow spanning multiple edge nodes using Docker containers.

## Infrastructure Overview

The `docker-compose.yaml` file defines the following services:
- **bridge**: The central coordination point for the workflow.
- **edge-a**: A radical.edge instance named `edge_a`.
...
```shell
export RADICAL_EDGE_TAG=makeflow
export RADICAL_EDGE_BRANCH=feature/task-dispatcher
```
...
2. **Wait for services to be ready:**
   The edge services depend on the bridge being healthy. You can monitor the startup process with:

   ```bash
   docker compose logs -f
   ```
   Wait until you see messages indicating that `edge_a` and `edge_b` have successfully connected to the bridge. Press `Ctrl+C` to stop following logs.
...
3. **Run the workflow:**
   ...
   ```bash
   docker exec -it radical-edge-client radical-edge-makeflow workflow.makeflow
   ```

   The `radical-edge-makeflow` script will:
   - Preprocess the workflow.
   - Dispatch tasks to the appropriate edge nodes (`edge_a` or `edge_b`).
   - Manage cross-edge file transfers through the client.

4. **Verify the output:**
   After the workflow completes, check for the generated `summary.txt`:

   ```bash
   docker exec radical-edge-client cat summary.txt
   ```
...
- **Development:** To use a specific branch of `radical.edge`, set the `RADICAL_EDGE_BRANCH` environment variable:
  ```bash
  RADICAL_EDGE_BRANCH=feature-branch docker compose build
  ```
- **Image Overrides:** You can use a custom image by setting `RADICAL_EDGE_IMAGE` and `RADICAL_EDGE_TAG`.
...
**Available Options:**
- `-b <branch>`: The `radical.edge` branch to clone and install (default: `master`).
