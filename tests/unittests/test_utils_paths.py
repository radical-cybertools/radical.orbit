"""Path resolution for broker cert/key: ``~`` must be expanded.
...
def test_cli_tilde_path_is_expanded(tmp_path, monkeypatch):
    ...
    path, source = utils._resolve_path_value(
        '~/.radical/orbit/broker_cert.pem', 'UNUSED_ENV', Path('/nope'))
    assert source == 'cli'
    assert path == tmp_path / '.radical' / 'orbit' / 'broker_cert.pem'
    assert '~' not in str(path)


def test_env_tilde_path_is_expanded(tmp_path, monkeypatch):
    monkeypatch.setenv('HOME', str(tmp_path))
    monkeypatch.setenv('ORBIT_TEST_PATH', '~/broker_key.pem')
    path, source = utils._resolve_path_value(
        None, 'ORBIT_TEST_PATH', Path('/nope'))
    assert source == 'env'
    assert path == tmp_path / 'broker_key.pem'
