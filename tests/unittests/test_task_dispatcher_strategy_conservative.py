...
class _Harness:
    ...
    Mirrors the two read methods :class:`PoolState` exposes to the policy:
    ``pending_queue()`` (QUEUED tasks, priority-ordered) and ``live_pilots()``
    (non-terminal pilots) — plus a ``submit_pilot`` callable to pass into
    ...
    def submit_pilot(self, size_key: str | None) -> str:
