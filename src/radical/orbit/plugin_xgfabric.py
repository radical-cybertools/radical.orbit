...
The plugin runs on a local endpoint and communicates with remote endpoints
(UCSB, Perlmutter) via the broker.
'''
...
class ResourceConfig:
    """Resource configuration — broker connection and per-cluster scheduler settings."""
    name: str = "default"
...
class XGFabricSession(PluginSession):
    ...
    async def _resolve_path(self, endpoint_name: str, path: str) -> str:
        ...
        if endpoint_name not in self._homedir_cache:
            try:
                ...
                return path
        return path.replace('~', self._homedir_cache[endpoint_name], 1)

    def update_connected_endpoints(self, endpoints: Dict[str, Any]):
        """Update the cached list of connected endpoints."""
        log.debug("[XGFabric] Session %s: topology update — %d endpoints: %s",
                  self._sid, len(endpoints), list(endpoints.keys()))
    ...
    async def get_status(self) -> Dict:
        ...
        # Query connected endpoints from broker (if not running a workflow)
        if self._state.status != 'running':
            immediate, allocate = await self._get_connected_endpoints()
    ...
    async def _execute_workflow(self):
        ...
        rc = self._current_resource_config
        log.info("[XGFabric] _execute_workflow: starting — session.broker_url=%s",
                 self._broker_url)
        ...
        immediate_list, allocate_list = await self._get_connected_endpoints()
        ...
        if not immediate:
            raise RuntimeError(
                ...
                f"(online immediate: {[c['endpoint_name'] for c in immediate_list]}, "
                f"online allocate: {[c['endpoint_name'] for c in allocate_list]})"
            )
        ...
        if allocate:
            ...
            pilot_id = await self._submit_pilot(allocate, broker_url)
        ...
        if allocate and await self._is_endpoint_online(allocate):
    ...
    async def _discover_executor(self, ec) -> str:
        ...
        historical default) if the endpoint has no queue_info plugin or the
        query fails.
        """
        try:
    ...
    async def _stage_sensor_data(self, cluster: Dict, sensor_csv: Path):
        ...
        workflow_path = await self._resolve_path(cluster['endpoint_name'], cluster['workflow_path'])
    ...
    async def _migrate_data(self, source: Dict, dest: Dict, sim_results: List[str]):
        ...
        dest_workflow = await self._resolve_path(dest['endpoint_name'], dest['workflow_path'])
...
class PluginXGFabric(Plugin):
    ...
    def is_enabled(cls, app: FastAPI) -> bool:
        """XGFabric loads on endpoint nodes (login or compute) — not on the broker."""
    ...
    def __init__(self, app: FastAPI, workdir: Optional[str] = None):
        ...
        self._connected_endpoints: Dict[str, Any] = {}  # Cache of connected endpoints
    ...
    def _create_session(self, sid: str, **_) -> XGFabricSession:
        ...
        # Seed session with current topology so get_status() classifies endpoints correctly
        if self._connected_endpoints:
            session.update_connected_endpoints(self._connected_endpoints)
    ...
    async def on_topology_change(self, endpoints: dict):
        ...
        for name in curr - prev:
            plugins = list(endpoints[name].get('plugins', []))
            log.info("[XGFabric] Endpoint connected: %s  plugins=%s", name, plugins)
        for name in prev - curr:
            log.info("[XGFabric] Endpoint disconnected: %s", name)

        # Update all active sessions and push updated cluster list to clients
        for session in self._sessions.values():
            if isinstance(session, XGFabricSession):
                session.update_connected_endpoints(endpoints)
                if session._plugin:
