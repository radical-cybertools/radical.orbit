...
class StagingSession(PluginSession):
    ...
    Provides methods to transfer files between client and endpoint filesystems.
    """
    ...
    async def put_file(self, filename: str, content_b64: str,
        ...
        Args:
            filename: Absolute path (or ~/...) on the endpoint filesystem
...
class StagingClient(PluginClient):
    ...
    def put(self, src: str, tgt: str, overwrite: bool = False) -> dict:
        """
        Upload a local file to the remote endpoint filesystem.

        Args:
            src: Absolute path on the local (client) filesystem
            tgt: Absolute path (or ~/...) on the remote (endpoint) filesystem
    ...
    def get(self, src: str, tgt: str) -> dict:
        ...
        Args:
            src: Absolute path (or ~/...) on the remote (endpoint) filesystem
    ...
    def list(self, path: str) -> dict:
        """
        List contents of a directory on the remote endpoint filesystem.

        Args:
            path: Absolute path (or ~/...) on the remote (endpoint) filesystem
...
class PluginStaging(Plugin):
    """
    Staging plugin for ORBIT.

    Provides file transfer between client and endpoint filesystems.
    ...
    ui_config = {
        "icon"       : "📁",
        "title"      : "File Staging",
        "description": "Transfer files between client and endpoint filesystems."
    }
    ...
    async def put_endpoint(self, request: Request) -> dict:
        """
        Upload a file to the endpoint filesystem.
    ...
    async def get_endpoint(self, request: Request) -> dict:
        """
        Download a file from the endpoint filesystem.
    ...
    async def list_endpoint(self, request: Request) -> dict:
        """
        List contents of a directory on the endpoint filesystem.
