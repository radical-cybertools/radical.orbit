...
`radical.orbit.EndpointRuntime` (exported as `Endpoint`) is a library.
...
One `EndpointRuntime` dials the broker over a **single
...
serving *and* consuming from inside one process — is the workflow-manager-in-an-allocation the star is built for: a runtime inside a batch job both offers its
...
`start` brings up the loops and connects;
`stop` cleanly closes the WebSocket (the
...
With `wait=True` (the default) `start` blocks until the first
`register_ack` **and** the first topology frame land, so on return
`topology` already reflects the broker's
snapshot and an immediate `get_plugin` will
...
- Call `serve` with a plugin class or
...
participant's plugin with `get_plugin`
...
::: radical.orbit.runtime.EndpointRuntime
    options:
...
`radical.orbit.Plugin` that defines a `plugin_name` class attribute is
...
`serve`. Because
`serve` constructs the plugin as
...
`PluginPSIJ` submits and manages jobs via the
[psij-python](https://exaworks.org/psij-python/) library, giving a unified API
...
See [REST API](rest_api.md) for the underlying request/response shapes and
[Plugin Development](plugin_development.md) for the full plugin authoring model.
